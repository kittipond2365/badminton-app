        ...
        for m in reversed(my_matches):
            ...
            for pid, pname in zip(my_ids, my_names):
                if pid != uid:
                    if pid not in partners:
                        partners[pid] = {'played': 0, 'won': 0, 'name': pname}
            ...
            for pid, pname in zip(opp_ids, opp_names):
                if pid not in opponents:
                    opponents[pid] = {'played': 0, 'lost': 0, 'name': pname}
...
def get_dashboard():
    try:
                ...
                for uid in m.get('team_a_ids', []):
                    if uid in db['players']:
                        m['team_a_data'].append({
                            "name": db['players'][uid]['nickname'],
                            "pic": db['players'][uid].get('pictureUrl', '')
                        })
                m['team_b_data'] = []
        ...
        # 2. Process Players List (Safe extraction)
        players_list = []
            ...
            for pid in e.get('players', []):
                if pid in db['players']:
                    joined_users.append({
                        ...
                        "pictureUrl": db['players'][pid].get('pictureUrl', '')
                    })
        ...
        # 6. All Players Data (Minimal for frontend)
        all_players_data = []
...
def matchmake():
    ...
    for p in active_players:
        ...
        if partner and partner.get('status') == 'active' and partner['id'] not in processed_ids:
            # Pair found
            t1 = float(p.get('last_active', 0))
            t2 = float(partner.get('last_active', 0))
            queue_time = max(t1, t2)
            ...
            groups.append({
                "type": "single",
                "members": [p],
                "queue_time": float(p.get('last_active', 0))
            })
            processed_ids.add(p['id'])
    ...
    def are_pair(u1, u2):
        return u1.get('partner_req') == u2['id'] or u2.get('partner_req') == u1['id']
    ...
    if not team_a:
        # Sort by MMR for balance if no pair
...
def submit_result():
    ...
    for uid in win_ids:
        if uid in db['players']:
            old = int(db['players'][uid].get('mmr', 1000))
            new = old + 25
...
def manual_matchmake():
    ...
    is_mod = req_uid in db['mod_ids']
    if req_uid != SUPER_ADMIN_ID and not is_mod:
        return jsonify({"error": "Unauthorized"}), 403
        
    court_id = int(d['courtId'])
    p_ids = d['playerIds']
    
    if active_courts.get(court_id):
        return jsonify({"error": "Court not empty"})
        
    players = [db['players'][pid] for pid in p_ids if pid in db['players']]
    if len(players) != 4:
        return jsonify({"error": "Need 4 valid players"})
...
def toggle_session():
    ...
    if uid != SUPER_ADMIN_ID and not is_mod:
        return jsonify({"error": "Unauthorized"}), 403
...
def manage_mod():
    ...
    if action == 'promote':
        if target_id not in db['mod_ids']:
            db['mod_ids'].append(target_id)
    else:
        if target_id in db['mod_ids']:
            db['mod_ids'].remove(target_id)
    save_data()
    return jsonify({"success": True})
...
def reset_system():
    ...
    for pid in db['players']:
        db['players'][pid]['mmr'] = 1000
        db['players'][pid]['status'] = 'offline'
